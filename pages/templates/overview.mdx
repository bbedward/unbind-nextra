...
Unbind has a built-in templating system that can compose multiple multiple services together, for example deploying a stack that requires a database, a web server, and a queue.
...
- **Docker Images** - Any docker image can be part tof a template
...
- **Variable References** - Template services can reference variables from other services withiin the same template (such as referencing the database credentials)
